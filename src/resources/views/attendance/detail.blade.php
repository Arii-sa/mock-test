...
<div class="detail-container">
    ...
    <form action="{{ route('attendance.request', $attendance->id != 0 ? $attendance->id : $attendance->work_date->format('Y-m-d')) }}" method="POST">
    @csrf

        {{-- 名前 --}}
        <div class="detail-row">
            <label>名前</label>
            <div class="value-box">{{ Auth::user()->name }}</div>
        </div>
        ...
        {{-- 修正ボタン（承認待ちなら非表示） --}}
        @if(empty($latestCorrection) || $latestCorrection->applications_status_id !== 1)
